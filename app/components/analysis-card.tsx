...
export default function AnalysisCard({ title, description, iconType, data, additionalMetrics }: AnalysisCardProps) {
  const rawRiskScore = typeof data.riskScore === 'string' 
    ? parseInt(data.riskScore.split('/')[0]) 
    : data.riskScore;
    
  const riskScore = title.includes('Dusting') 
    ? Math.round(rawRiskScore * 100) 
    : Math.round(rawRiskScore * 100);
    
  const isHighRisk = riskScore >= 30;
  const isMediumRisk = riskScore >= 20 && riskScore < 30;
            ...
            <div className="bg-red-950/40 border-2 border-red-500 rounded-lg p-3 sm:p-4 mb-4 relative overflow-hidden">
              ...
              {data.mimickedAddress && (
                <div className="mt-4 p-3 bg-red-900/40 border border-red-500/60 rounded">
                  <p className="text-xs sm:text-sm text-red-300 font-semibold mb-2">🎯 TARGET ADDRESSES BEING MIMICKED:</p>
                  {Array.isArray(data.mimickedAddress) ? (
                    data.mimickedAddress.map((address, index) => (
                      <div key={index} className="bg-red-800/40 rounded p-2 mb-2">
                        <span className="text-xs text-red-200 font-mono break-all leading-relaxed">{address}</span>
                      </div>
                    ))
                  ) : (
